...
<script lang="ts">
  ...
  const brushPresets = [5, 10, 20, 40, 80];
  
  function setBrushPreset(size: number) {
    refinementActions.setBrushSize(size);
    dispatch('settingsChanged', { brushSize: size });
  }
</script>
    ...
    <div class="control-group">
      ...
      <div class="preset-buttons">
        {#each brushPresets as preset}
          <button
            on:click={() => setBrushPreset(preset)}
            class="preset-btn"
            class:active={brushSize === preset}
            title="Set brush size to {preset}px"
          >
            {preset}
          </button>
        {/each}
